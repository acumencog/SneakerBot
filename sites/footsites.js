...
async function checkout({
  ...
  try {
    ...
    const cardExpirationMonthFrame =
      await cardExpirationMonthFrameHandle.contentFrame();
    ...
    const cardExpirationYearFrame =
      await cardExpirationYearFrameHandle.contentFrame();
...
async function closeModal({ taskLogger, page }) {
  try {
    ...
    await modal.evaluate(() => {
      document
        .querySelector(
          'button.closeButtonWhite, button[name="bluecoreCloseButton"]'
        )
        .click();
    });
  } catch (err) {
...
async function searchByProductCode({ taskLogger, page, productCode, domain }) {
  ...
  let searchResult;
  while (!searchResult) {
    await page.goto(`${domain}/search?query=${productCode}`, {
      waitUntil: "domcontentloaded",
    });
    await Promise.all([
      (async () => {
        try {
          searchResult = await page.waitForSelector(
            ".SearchResults .ProductCard a.ProductCard-link",
            { timeout: 5 * 1000 }
          );
      ...
      (async () => {
        try {
          searchResult = await page.waitForSelector("#ProductDetails", {
            timeout: 5 * 1000,
          });
...
exports.guestCheckout = async ({
  ...
  try {
    ...
    while (!isInCart && !hasCaptcha) {
      ...
      // const sizesSelector = 'Button.SizeSelector-button.SizeSelector-button';
      const sizesSelector = "Button.SizeSelector-button";
      ...
      await page.waitForFunction(
        ({ selector, sizeStr }) => {
          ...
          const matchingSizeDiv = sizeDivs.find((el) =>
            new RegExp(sizeStr, "i").test(el.innerText)
          );

          if (matchingSizeDiv) {
            matchingSizeDiv.click();
            return true;
          }
          return false;
        },
